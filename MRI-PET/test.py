...
def load_model(path_auto, flag_img=False):
	...
	print(fusion_model)
	net = torch.load("models/train/fusionnet/fusion_axial/PET_MRI_output.model")
...
def main():
	# ################# gray scale ########################################CT_other
	test_path = "images/PET/"
	path_auto = args.resume_nestfuse
	output_path_root = "./outputs/PET_MRI_1/"
	...
	with torch.no_grad():
		...
		for i in range(1):
			...
			for i in range(num):
				...
				vis_path = ir_path.replace('PET\\', 'MRI\\')
				if vis_path.__contains__('PET'):
					vis_path = vis_path.replace('PET', 'MRI')
				else:
