...
def list_images(directory):
    ...
    for dir0 in dir:
        ...
        #print(os.listdir(os.path.join(directory, dir0)).sort())
        for dir1 in os.listdir(os.path.join(directory, dir0)):
            req_path = os.path.join(directory, dir0, dir1, 'SPECT')
            for file in os.listdir(req_path):
                name = file
                if name.endswith('.png'):
                    images.append(join(req_path, file))
...
def get_test_image(paths, height=256, width=256, flag=False):
    ...
    for path in paths:
        ...
        # get saliency part 显著
        if height is not None and width is not None:
            #image = imresize(image, [height, width], interp='nearest')
            # ：[https://baike.baidu.com/item/imresize/9948188?fr=aladdin]
            # parameter: 1.image:图片,2.:目标大小,3.:采用插值方法 (此处采用3次插值法)
            image = imresize(image, [height, width], interp='bicubic')
...
if __name__ == '__main__':
    ...
    imgs_paths_ir, names = Mylist_images(test_path)
    #print(imgs_paths_ir)
